...
Normally an orphan re-attaches when the same file is re-indexed (same uuid). But combined with the path-based `stable_uuid`, a library-path change re-keys every uuid, so every override row orphans permanently with no possibility of re-link and no GC. Dead rows also slow the `LEFT JOIN metadata_overrides` in the browse/discovery count queries (`db/src/browse.rs:52,150`; `db/src/discovery/authors.rs:67`). The same orphan-forever shape will apply to F3.2 ratings/journals if they correctly use the soft-ref pattern, so a reusable reconcile/GC is needed regardless.

Fix direction: a periodic or post-reindex reconcile that deletes override rows whose `book_uuid` has no matching `books.uuid` AND no `merged_uuids.uuid` (with a grace window so legitimately-detached-pending-reindex rows survive), unlinks orphaned cover files, and surfaces detached overrides as "unlinked" per the F3.2 idea. At minimum, prune override rows + cover files when the user explicitly removes a library root (`db/src/settings.rs:147-152`).
