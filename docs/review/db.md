...
**Resolved** ([db-review-f10-override-gc.md](../design/db-review-f10-override-gc.md)): a **missing-files GC**. Migration 0029 adds `is_missing_files` / `missing_files_since` / `is_missing_files_override` to `books`; `mark_book_files_missing` stamps the flag and the file-write chokepoints clear it. `missing_files::gc_books_missing_files`, run best-effort after each reindex, hard-deletes books missing past a 30-day retention that carry **no** user data (any 0027 soft-ref table), no `merged_uuids` attachment, and no wishlist override — taking their regenerable `metadata_overrides` row + cover files with them. Books a user interacted with are kept indefinitely; the "unlinked" admin UX stays deferred to F3.2.
