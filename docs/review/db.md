...
But F3.1 (`docs/roadmap/3-1-libraries.md:19`) defines a different, user-facing concept also called "libraries" — named saved metadata-filter collections: "a `libraries` table + `library_filter_rules` child table with `(library_id, field, op, value)` rows", described as "the core v1 differentiator (v1 #3)." The cascade semantics differ fundamentally: deleting a scan root must prune its books; deleting a saved filter-collection must delete nothing but rule rows. The two cannot share the name.
...
`ORDER BY b.sort, b.id` has no supporting library-scoped composite index (`idx_books_sort` is global, so it can't both filter by library and provide the sort), forcing a temp-sort. This works at current sizes but is the join-fan-out shape that won't scale, and F3.1 libraries-as-filters will wrap this projection in more predicates. Fix direction: collapse the duplicated subquery pairs into `json_object` selects; add a `(library_id, sort, id)` index for the sort; add real keyset pagination before libraries grow; consider a denormalized book-card read-model materialized by the indexer for the hot path.
...
### Medium Priority — metadata_overrides has no FK and no GC

`metadata_overrides` is keyed by `book_uuid TEXT PRIMARY KEY` with no FK to books — intentional, so user edits survive the indexer's DELETE/INSERT reindex cycle (`0007_metadata_overrides.sql:16-22`, comment at `db/src/sync/books.rs:77-80`). The trade-off: nothing ever removes a row whose book is permanently gone. The only deletes are user-driven (`db/src/metadata_overrides/upsert.rs:175`) or merge-driven (`db/src/merge/transaction.rs:409`). When a book's file is permanently removed (Removed bucket cascade-deletes the `books` row) or a whole library is pruned, the matching override row is left behind forever — and each orphan also carries a leaked override cover file (`override-<uuid>.<ext>`).

Normally an orphan re-attaches when the same file is re-indexed (same uuid). This was originally compounded by the path-based `stable_uuid`, where a library-path change re-keyed every uuid and orphaned every override row permanently — but **F2 has since landed** (durable stored `books.uuid` + relative-path `scan_key` diff), so a repoint now preserves every uuid and an orphan genuinely means "the file is gone." Dead rows also slow the `LEFT JOIN metadata_overrides` in the browse/discovery count queries (`db/src/browse.rs:52,150`; `db/src/discovery/authors.rs:67`). The same orphan-forever shape will apply to F3.2 ratings/journals if they correctly use the soft-ref pattern, so a reusable reconcile/GC is needed regardless.

**Decided** ([db-review-f10-override-gc.md](../design/db-review-f10-override-gc.md)): **Option C (soft-detach)**. A post-reindex *and* explicit-library-removal reconcile marks orphan rows (`book_uuid` absent from both `books.uuid` and `merged_uuids.uuid`) with a new `detached_at` column instead of deleting them — filtered out of the read path immediately, cover files swept, and hard-purged only after a 30-day retention. On the reindex path a detached row re-links automatically if the file returns (same uuid); rows detached by explicit library removal can't re-link (re-add mints fresh uuids) and simply age out. The admin "unlinked" surface is deferred to F3.2 so it lands with "unlinked annotations." The per-reindex arm is safe now that F2 has removed the mass-orphan hazard.
