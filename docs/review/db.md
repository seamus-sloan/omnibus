...
**Decided** ([db-review-f10-override-gc.md](../design/db-review-f10-override-gc.md)): **Option C (soft-detach)**. A post-reindex *and* explicit-library-removal reconcile marks orphan rows (`book_uuid` absent from both `books.uuid` and `merged_uuids.uuid`) with a new `detached_at` column instead of deleting them — filtered out of the read path immediately, re-linkable if the book returns, cover files swept, and hard-purged only after a 30-day retention. The admin "unlinked" surface is deferred to F3.2 so it lands with "unlinked annotations." The per-reindex arm is safe now that F2 has removed the mass-orphan hazard.
