...
Symmetrically, the upload/edit endpoints check `can_edit` but never `can_upload`: `post_ebook_cover` (`overrides.rs:110-115`) and the cover/photo PUTs the route comment frames as "binary-upload endpoints" (`backend.rs:48-58`) all gate on `can_edit`, so the column an admin would toggle to revoke upload rights does nothing.
...
### High Priority — RPC server functions leak raw DB errors
...
Explicit leaks: `rpc_save_progress` maps `ProgressError::Sqlx(e)` to `ServerFnError::new(e.to_string())` (`frontend/src/rpc.rs:549`); every highlight RPC does the same for `HighlightError::Sqlx(e)` plus a catch-all `Err(e)` arm (`rpc.rs:603-605,613-617,627-631,641-645,655-659`); `rpc_set_author_photo_url` stringifies a raw sqlx error from its author-exists query (`rpc.rs:425`) and from upsert (`rpc.rs:444`). Implicit leaks: the read RPCs (`rpc_get_settings`, `rpc_get_ebooks`, `rpc_get_author`, `rpc_list_authors`, `rpc_search`, etc.) use a bare `?` on db calls (e.g. `rpc.rs:149,199,224,239,327,451,462`), so the `Display` of the underlying `SettingsError`/anyhow/sqlx error crosses the wire via the `ServerFnError` `From` conversion and lands in `DataError::Other(e.to_string())` at `frontend/src/data.rs:513`.

Sqlx error `Display` can include SQL fragments, column names, and constraint detail — information disclosure to any authenticated web client, and inconsistent with the carefully-generic REST path (the same failure returns a clean 500 on mobile but a detailed string on web). Fix: give the RPC layer an `internal()`-equivalent that logs server-side and returns a fixed opaque `ServerFnError` for all sqlx/anyhow failures, reserving per-case messages only for typed/validation variants clients legitimately branch on (`BookNotFound`, `NotFound`, validation).
