...
### High Priority — RPC server functions leak raw DB errors (explicit leaks fixed in #761; implicit leaks remain)

The REST surface is disciplined: every backend handler funnels unexpected failures through `internal()` (`server/src/backend.rs:63-70`), which logs the full error via `tracing::error!` and returns a fixed "internal server error" body, so sqlx/driver internals never reach the wire. The `/api/rpc/*` server functions used to do the opposite; `rpc.rs` has since been split into `frontend/src/rpc/*.rs` submodules (this note's old `rpc.rs:NNN` citations are stale).

Explicit leaks — **fixed** (#761): every stringified `Sqlx`/catch-all error arm across `frontend/src/rpc/{journals,highlights,ratings,progress,account,settings,kindle}.rs`, plus the `fetch_remote_image` call in `authors.rs::rpc_set_author_photo_url`, now routes through `internal_rpc_error` (`frontend/src/rpc/mod.rs`), which logs the real error via `tracing::error!` and returns a fixed opaque `ServerFnError`, mirroring `server/src/backend.rs::internal`. Two adjacent sqlx-error-to-string sites in the same function (`authors.rs::rpc_set_author_photo_url`'s author-exists check and `upsert_author_photo` call) were left as-is — same pattern, but outside #761's cited evidence; worth a quick follow-up.

Implicit leaks — still open: the read RPCs (`rpc_get_settings`, `rpc_get_ebooks`, `rpc_get_author`, `rpc_list_authors`, `rpc_search`, etc.) use a bare `?` on db calls across `frontend/src/rpc/*.rs`, so the `Display` of the underlying `SettingsError`/anyhow/sqlx error still crosses the wire via the `ServerFnError` `From` conversion and lands in `DataError::Other(e.to_string())` at `frontend/src/data.rs:513`. Sqlx error `Display` can include SQL fragments, column names, and constraint detail — information disclosure to any authenticated web client, and inconsistent with the carefully-generic REST path (the same failure returns a clean 500 on mobile but a detailed string on web). Fix: extend `internal_rpc_error` (or an equivalent `map_err`) to the bare-`?` read paths, reserving per-case messages only for typed/validation variants clients legitimately branch on (`BookNotFound`, `NotFound`, validation).
