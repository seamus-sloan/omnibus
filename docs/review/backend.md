...
Explicit leaks — **fixed** (#761): every stringified `Sqlx`/catch-all error arm across `frontend/src/rpc/{journals,highlights,ratings,progress,account,settings,kindle,authors}.rs` now routes through `internal_rpc_error` (`frontend/src/rpc/mod.rs`), which logs the real error via `tracing::error!` and returns a fixed opaque `ServerFnError`, mirroring `server/src/backend.rs::internal`. This includes all three sqlx/transport-error sites in `authors.rs::rpc_set_author_photo_url` (author-exists check, `fetch_remote_image`, `upsert_author_photo`).
