...
settings.rs is documented "Admin-only" (line 4) but the component renders the full library-path form, Save, Refetch-author-photos, and Extract-chapters buttons to any authenticated user — there is no `is_admin` check anywhere in the component body. It is reachable by everyone: the mobile bottom nav links Settings for all (bottom_nav.rs:19) and the web user-menu Settings row is enabled for all (user_menu.rs:223-229). A non-admin loads a fully interactive admin form and only discovers it is forbidden when an action 403s server-side — a confusing dead end with no up-front "you don't have access" messaging. The book-detail page already demonstrates the right pattern: it resolves `current_user()` into an `is_admin` signal to gate the Merge affordance (book_detail.rs:58-65).
