//! `POST`/`DELETE /api/ebooks/:uuid/cover` — override cover upload,
//! rejection paths (non-image, oversized, undetectable format), and
//! revert-to-scanned-cover, including the orphan-file cleanup/preserve
//! behaviour when the DB upsert fails mid-write.
