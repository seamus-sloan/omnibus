...
#[tokio::test]
async fn api_get_stats_ignores_an_out_of_range_offset_rather_than_400ing() {
    let (app, _state, pool) = fixture().await;
    let user = auth_test_support::create_user(&pool, "alice").await;
    let token = auth_test_support::bearer_token(&pool, user.id).await;

    // A bad offset must not cost a reader their stats page: it is discarded in
    // favour of the fallback, not rejected. `db::user_offset` owns that call.
    let res = app
        .oneshot(get_with_bearer(
            "/api/stats?range=all_time&utc_offset_minutes=99999",
            &token,
        ))
        .await
        .unwrap();
