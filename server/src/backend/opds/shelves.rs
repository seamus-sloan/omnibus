//! Shelves browse: `/opds/shelves` (navigation feed of the viewer's visible
//! shelves) and `/opds/shelves/{id}` (one shelf's acquisition feed, title
//! order). Listing goes through `db::list_visible_shelves`, with per-shelf
//! access re-checked by `db::can_view` — a non-visible shelf 404s, never 403s.
//! This is the one surface where the *shelf itself* is the thing being gated.
