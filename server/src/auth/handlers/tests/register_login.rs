//! The session cookie's secure flag and `__Host-` prefix, registration
//! (first user becomes admin, bearer tokens, password / username
//! validation, duplicates, the disabled gate), login (cookie, wrong
//! password, lockout with `Retry-After`, unknown user, corrupt hash), and
...
fn session_cookie_name_helper_branches_on_secure_flag() {
    assert_eq!(
        super::super::session_cookie_name(true),
        "__Host-omnibus_session"
    );
    assert_eq!(super::super::session_cookie_name(false), "omnibus_session");
}
...
async fn register_first_user_becomes_admin_and_sets_cookie() {
    ...
    let expected_name = format!("{}=", super::super::session_cookie_name(true));
    assert!(
