...
hls.rs              — F2.3 HLS transcode cache (fallback path after #339): `resolve_audiobook`, `get_parts`, `build_manifest` (VOD m3u8 from stored durations), `transcode_book` (ffmpeg concat→AAC-LC 64 kbps mono, 10 s segments), `evict_if_over_cap` (FIFO-by-mtime). Only invoked for non-natively-playable codecs (flac/ac3/eac3/…); m4b/m4a/mp3/aac direct-play via `/api/audiobooks/:uuid/parts/:ordinal`. Env: `OMNIBUS_DATA_DIR` (segment root), `OMNIBUS_HLS_CAP_BYTES` (default 5 GiB), `OMNIBUS_FFMPEG_PATH`, `OMNIBUS_HLS_TRANSCODE_TIMEOUT_SECS`.
...
backend/audiobooks.rs — F2.3 audiobook playback routes: `GET /api/audiobooks/{uuid}/manifest` returns `{mode:"direct", parts:[…]}` (m4b/m4a/mp3/aac — instant playback) or `{mode:"hls", playlist_url}` (flac/ac3/… — falls back to transcode) per `db::audiobook::codec::classify_filenames`. `GET /api/audiobooks/{uuid}/parts/{ordinal}` Range-serves source files via `tower_http::ServeFile`. Legacy HLS routes (`/playlist.m3u8`, `/segments/{seg}`, `/status`) retained as the fallback path. See #339.
