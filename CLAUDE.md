...
worker.rs           — single-process Worker primitive: scan_sem (Scan/ScanAudiobooks) + hls_sem (HlsTranscode) semaphores + per-resource keyed mutex; Task::HlsTranscode dispatches into hls::transcode_book; WorkerConfig::hls_concurrency defaults to max(1, num_cpus/2). F1.11 ResolveAuthorPhoto dispatches into author_photos::resolve.
