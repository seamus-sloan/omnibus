...
- **Those four prefixes are the whole allowed set.** A global `commit-msg` hook (personal config, via `core.hooksPath`) rejects any other subject — `docs:`, `test:`, `refactor:`, `ci:` and friends all bounce, so fold them into `chore:`. Merge, revert, and `fixup!` / `squash!` / `amend!` subjects are exempt, and `git commit --no-verify` bypasses it once. Being a *local* hook it never sees a GitHub squash-merge, which is why `main`'s history still carries older `docs:` subjects taken from PR titles.
