...
use views::{
    BookProgressView, BookmarkView, HighlightView, JournalEntryView, PhysicalCopyView,
    ProgressRecordView, ReadStatusView, ResumePointView, SessionLogEntryView, SessionLogPageView,
};
...
impl OmnibusMcp {
    ...
    #[tool(
        description = "Fetch one book's full metadata by uuid, including its on-disk files (book_files) with per-file formats, sizes, and — for audio files — `duration_seconds`, the file's playing time in whole seconds. Pass include to fold this reader's state into the same answer — any of progress, read_status, highlights, bookmarks, sessions, copies — instead of making a call per section. Every timestamp comes back as an ISO 8601 string with its unix-seconds twin alongside it under the same name plus _epoch."
    )]
    pub async fn get_book(
        ...
        if include.contains(&BookInclude::Progress) {
            detail.progress = Some(self.progress_every_format(&p.uuid).await?);
        }
    ...
    /// reader has never opened. One read: the endpoint answers with the whole
    /// envelope unless a `format` narrows it deliberately.
    async fn progress_every_format(
        &self,
        uuid: &str,
    ) -> Result<Vec<ProgressRecordView>, ErrorData> {
        let path = format!("/api/progress/{uuid}");
        let envelope: Option<BookProgress> = self.client.get_json(&path, &[]).await?;
        Ok(envelope
            .map(|e| e.records.into_iter().map(Into::into).collect())
            .unwrap_or_default())
    }
