...
) -> AxumJson<Option<BookProgress>> {
    if uuid != BOOK {
        return AxumJson(None);
    }
    // This reader has opened the EPUB and never the audiobook, so the
    // envelope carries one record — the case that proves a format with no
    // position is skipped rather than reported empty.
    let records = match q.format.as_deref() {
        Some("audio") => Vec::new(),
        _ => vec![progress_record(ProgressFormat::Epub)],
    };
    AxumJson(Some(BookProgress {
        book_uuid: BOOK.into(),
        furthest: records.first().map(|r| r.format),
        records,
        linked: false,
        cross_format: None,
    }))
...
async fn highlights(Path(_): Path<String>) -> AxumJson<Vec<Highlight>> {
    AxumJson(vec![Highlight {
        ...
        created_at_iso: None,
        spine_index: Some(6),
        chapter_title: Some("The Vestibule".into()),
        percent_through_book: Some(37.5),
    }])
}
...
#[test]
fn iso_renders_unix_seconds_as_an_iso8601_utc_stamp() {
    assert_eq!(iso(WHEN), "2026-03-04T00:00:00Z");
    assert_eq!(iso(0), "1970-01-01T00:00:00Z");
}

#[test]
fn iso_renders_every_epoch_so_a_stamp_is_never_missing() {
    // The shared conversion is total over i64, so a stamp paired with a
...
async fn book_progress_carries_each_stamp_as_iso_beside_its_epoch() {
    ...
    let record = envelope
        .records
        .first()
        .expect("the envelope carries the record");
    assert_eq!(record.updated_at, "2026-03-04T00:00:00Z");
    assert_eq!(record.updated_at_epoch, WHEN);
    assert_eq!(record.client_updated_at, "2026-03-03T23:59:00Z");
...
async fn every_record_tool_answers_with_an_iso_stamp_for_each_epoch_it_returns() {
    ...
    let sessions = service
        ...
        .unwrap()
        .0;
    assert_eq!(sessions.entries[0].started_at, "2026-03-04T00:00:00Z");
    assert_eq!(sessions.entries[0].started_at_epoch, WHEN);
    assert_eq!(sessions.entries[0].ended_at, "2026-03-04T00:30:00Z");
