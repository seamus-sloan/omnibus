...
- `OMNIBUS_PUBLIC_ORIGIN=http://localhost:$PORT` — comma-separated allowlist consumed by `auth::origin_check`. Required for `dx serve --fullstack`: its HTTP proxy rewrites `Host` to the upstream backend's loopback address without setting `X-Forwarded-Host`, so without an allowlist every cookie-authed POST 403s. Override in production deployments behind a reverse proxy. `just serve` / `just serve-pc` bind dx serve to `--addr 0.0.0.0` so other LAN devices can reach `http://<lan-ip>:3000`; to actually authenticate from a LAN client, override this var to include the LAN origin and set `OMNIBUS_SECURE_COOKIES=0` (see [.env.example](../../.env.example)).
