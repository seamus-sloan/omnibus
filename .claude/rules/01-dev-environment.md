...
- `RUST_LOG` — console log filter (tracing `EnvFilter` syntax). The server owns its subscriber (`init_tracing` in `server/src/logging.rs`), so this works under both `cargo run -p omnibus` and `dx serve`. Default when unset: `info,omnibus=debug` — one INFO line per HTTP request (method, path, status, latency, user agent; query strings never logged). The same filter gates both the stderr and the on-disk JSON sinks.
