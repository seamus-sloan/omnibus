...
- `OMNIBUS_EXPLORE_URL` / `OMNIBUS_EXPLORE_ADMIN` — the persistent instance the [agentic-exploration](../skills/agentic-exploration/SKILL.md) swarm drives, and the single credential that system persists (`user:password`). Per-agent accounts keep **stable usernames** because provenance ownership is keyed on the actor — a fresh account each run would orphan every book earlier runs uploaded — while their passwords are rotated at each invocation through the admin reset endpoint and never stored. Point it at a throwaway instance only: agents upload, edit metadata, merge, and delete. Optional siblings `OMNIBUS_EXPLORE_SSH_HOST`, `OMNIBUS_EXPLORE_REMOTE_DIR`, `OMNIBUS_EXPLORE_SNAPSHOT_DIR`, `OMNIBUS_EXPLORE_SNAPSHOT_KEEP` tell `scripts/explore/snapshot.sh` where the instance host lives and how many tarballs to keep.
