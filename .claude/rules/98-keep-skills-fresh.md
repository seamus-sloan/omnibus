...
| `docs/qa/agentic_exploration/` (esp. the `flows/README.md` weight table the sampler parses), `scripts/explore/` (incl. `audit_lib/` — the skill quotes `audit.json`'s field names and the journal's location), `server/src/backend/users.rs` (account provisioning), `server/src/auth/handlers.rs` (login) | [agentic-exploration](../skills/agentic-exploration/SKILL.md) |
