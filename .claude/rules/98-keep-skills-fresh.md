...
| `ui_tests/playwright/tests/fixtures/`, `ui_tests/playwright/tests/utils/`, selector conventions (incl. [04a-playwright-surfaces.md](04a-playwright-surfaces.md)) | [add-playwright-flow](../skills/add-playwright-flow/SKILL.md) |
...
| `docs/qa/agentic_exploration/` (esp. the `flows/README.md` flow table the sampler parses — its `Runs` column), `scripts/explore/` (incl. `audit_lib/` — the skill quotes `audit.json`'s field names and the journal's location), `server/src/backend/users.rs` (account provisioning), `server/src/auth/handlers.rs` (login) | [agentic-exploration](../skills/agentic-exploration/SKILL.md) |
