...
components/merge_dialog.rs — F5.10 admin "Merge with…" dialog mounted by the book detail page (web-only): FTS-searched candidate list → confirm → POST /api/rpc/merge-books; the page's book is always the merge target (target metadata wins). Parent renders the undo toast (POST /api/rpc/merge-books/undo).
