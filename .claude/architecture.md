...
sync/               — transactional indexer write path: `books.rs` (sync_books buckets), `audiobooks.rs` (sync_audiobooks + parts/chapters), `authors.rs`, `backfill.rs`, and `attach/` — cross-format auto-attach (New-bucket title+author match or merged_uuids hit hangs a new file's book_files row off an existing book in another format; Changed refreshes it, Removed drops it leaving the book — possibly fileless, a legal state).
merge/              — F5.10 manual merge: `merge_books` (one-transaction move of book_files/links/identifiers/progress/history + overrides shallow-merge + cover adoption + merged_uuids guard) and `undo_merge` (recreates the source from the merge_log JSON snapshot, links restored by name). Exposed via /api/rpc/merge-books{,/undo}.
