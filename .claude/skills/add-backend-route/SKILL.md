...
- **Serving a file off disk:** go through `serve_download` (attachment) or `validator::serve_file` (inline) in `server/src/backend/`, never `ServeFile` directly. They stamp the content validator and evaluate `If-None-Match` / `If-Range`, and the `If-Range` half is not optional politeness: `tower_http::ServeFile` ignores that header, so a raw `ServeFile` answers a resumed download of a file that changed underneath it with a 206 counted from the *new* bytes — the client appends a new tail to an old head and stores the result as a finished book, with nothing raised anywhere. Never pair a file route with a bare `max-age`; a book replaced in place keeps its URL, so the client has to be able to ask.
