...
- **New page route:** extend the `Route` enum in [frontend/src/routes.rs](../../../frontend/src/routes.rs) and add a page component under `frontend/src/pages/`. Dioxus fullstack handles SSR + hydration automatically — no new handler required.
...
- **Image-serving GET whose bytes go into an `<img src>`** (covers/thumbs/photos): use `_user: MediaAuthUser` instead of `AuthUser`, and add the path prefix to `is_media_read_path` in `auth/gate.rs`. Both accept the session as a `?token=` query param — a mobile WebView's `<img>` fetch carries neither the bearer header nor a cookie. On the frontend build the URL via `crate::media_url` / `crate::thumb_url` (in `contexts.rs`) so the token is appended per URL, including each `srcset` candidate. Every cacheable 200 these return must publish an `ETag` — **including a stand-in body**, like the full cover `/api/thumbs/*` serves while the WebP generates. Both offline clients cache what comes back, and one that arrives with no validator is one they can never revalidate; see [09-content-validators.md](../../rules/09-content-validators.md).
- **GET that streams bytes off disk** (a book file, an audio part, a download): route it through `backend::conditional` so it publishes an `ETag`, answers `If-None-Match` with a 304, and refuses a `Range` whose `If-Range` went stale. `tower-http`'s `ServeFile` gives you an ETag but has **no `If-Range` support**, so delegating to it alone lets a resume splice the tail of a new file onto the head of an old one. See [09a-serving-validated-bytes.md](../../rules/09a-serving-validated-bytes.md) for the call shape and why `ETag` and `Vary` must travel together, and [09-content-validators.md](../../rules/09-content-validators.md) for what the validator itself is derived from.
- **POST that fetches a client-supplied URL server-side** (apply a provider cover, paste an author photo URL): never write a second fetcher — go through `db::author_photos::fetch_remote_image_with`, which is the one SSRF implementation in the workspace (IP-range guard before any connect, addresses pinned against DNS rebinding, streamed size cap). Tighten it with `RemoteImageConfig`: a `host_allowlist` derived from a catalog rather than hand-written, `require_https`, and `max_redirects` — every hop is re-gated, so following one is only safe through that path. Sniff the bytes with `detect_image_format` afterwards; a remote `Content-Type` is a claim. Mount it in `upload_router()` even though it carries no upload body: the outbound fetch is the cost the per-IP limiter is for. `overrides::post_ebook_cover_from_url` is the worked example — it reaches that fetcher through `db::fetch_provider_cover`, a thin wrapper that spends a *second* gated fetch to upgrade a Google Books cover past its 128px thumbnail and falls back when the larger rendition can't be shown to be the same picture. Wrapping like that is fine; a parallel fetcher is not.
...
- **Reads**: keep the raw REST body in a `pub(crate) *_online` fn and make the public fn a `crate::offline::cache::read_through(key, async move { *_online(...).await })` wrapper (the future needs owned args — `Send + 'static`), with the key added to `offline::cache::keys`. The policy is cache-first (stale-while-revalidate): cached copy served instantly, background revalidation bumps the generation channel pages watch via `use_cache_generation`. Use `read_through_network_first` only when acting on a stale answer would be a correctness bug (see `get_me` / `get_progress` for the two existing cases). See any read in `data/books/library.rs`.
