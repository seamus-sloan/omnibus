...
This is the canonical flow for validating any change to the Dioxus web UI. Uses **Codex Preview** (`mcp__Claude_Preview__preview_*`) — each agent gets its own isolated headless Chromium, so parallel agents across workspaces never share a browser. It assumes nothing about the current server state — safe to re-run.
...
- Starts `dx serve --platform web --fullstack --port <chosen>` in the background (daemonized so it survives the agent shell); output goes to `.Codex/runtime/server.log`, PID to `.Codex/runtime/server.pid`.
...
- Writes `.Codex/runtime/port` and `.Codex/runtime/env.sh`.
...
```bash
source .Codex/runtime/env.sh   # if not already sourced
...
- **`dev-up` exits 1 with "ports … all held by non-omnibus processes."** Run `lsof -iTCP:$PORT-$((PORT+9)) -sTCP:LISTEN -P` to see what's holding them. Most likely cause: a previous `dx serve` you forgot about — `cat .Codex/runtime/server.pid` and `just dev-down` (identity-checked), or pick a different starting `PORT`.
- **`dev-up` exits 2 with "server unhealthy — run `just dev-bounce`."** `dx serve` is up but its most recent rebuild errored. Read `.Codex/runtime/server.log` for the compile error, fix it, then `just dev-bounce`.
