...
//! Pages read (the ground actually covered in the window, each book's length
//! resolved through the ladder in `db::stats::pages`, which prefers a real
//! print page count and falls back to a word estimate), and Listening. All four
//! are period-scoped and re-render when the switcher changes, and each carries
//! a pull-to-expand grip that opens its drill-in (`drill_in.rs`).
...
/// The four-tile headline row. Finished carries the accent tint; Pages read is
/// how much of each book was actually covered in the window, scaled by a length
/// that is exact for a comic or a print-edition override and an estimate
/// otherwise — the drill-in carries that caveat, since the headline has no room
/// for it. Takes only the scalar fields it renders — never the whole
/// `StatsSummary` — so a re-render doesn't clone the DTO's heatmap /
/// finished-books vectors. `expanded` is set by a tile's grip to open its
/// drill-in (`drill_in.rs`).
#[component]
pub(super) fn HeadlineTiles(
    books_finished: i64,
    avg_stars: Option<f64>,
    pages_read: Option<i64>,
    pages_audio_only: bool,
    ...
    let pages = pages_value(pages_read, pages_audio_only);
