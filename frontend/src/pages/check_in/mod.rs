...
//! plus title search) is the front door, with the camera scanner behind an
//! explicit button; the mobile shell inverts that. All of them feed the same
//! [`Stage`] machine. Every screen is plain rsx with no target gating, so SSR
...
pub(crate) enum Stage {
    ...
    /// mobile shell, and the title-search route back from [`Stage::Unresolved`].
    Lookup,
...
/// Its title-search route goes to [`Stage::Lookup`] rather than the front door
/// — on the mobile shell that's the camera, which has no search field.
