//! Tests for the self-registration switch: `registration_status_line`'s
//! subtitle copy, and the pure guards/failure-revert in
//! `registration_toggle_handler` / `apply_registration_load`, exercised
//! inside a rebuilt `VirtualDom` so `Signal::new` has a runtime without a
//! browser.
