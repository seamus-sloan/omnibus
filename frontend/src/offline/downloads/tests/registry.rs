//! The plan builders (`DlFormat`, the EPUB and audio-part plans, MIME /
//! extension round trips, size estimates, the default audio file) and the
//! registry itself: entry rows, status and downloaded-uuid reads, progress
//! updates, per-format file removal, AC4 isolation between books, and the
//! instant error row when offline.
