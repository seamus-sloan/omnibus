...
async fn img_handler(
    ...
    if !bust {
        if let Some(hit) = cached {
            ...
            revalidate_in_background(st.img_dir.clone(), path.clone(), &hit);
...
/// [`IMG_REVALIDATE_AFTER_SECS`], and while another revalidation of the same
/// path is already in flight — a grid scroll must not turn into one request
/// per visible cover, repeatedly.
fn revalidate_in_background(img_dir: PathBuf, path: String, cached: &CachedImage) {
    ...
    if cached.age_secs < IMG_REVALIDATE_AFTER_SECS
        || cached.etag.is_none()
    ...
    tokio::spawn(async move {
        ...
        match cached_image(&img_dir, &path).await {
            Some(current) if current.age_secs >= IMG_REVALIDATE_AFTER_SECS => {
                if let Some(etag) = current.etag.as_deref() {
                    let _ = fetch_and_cache(&img_dir, &path, Some(etag)).await;
                }
            }
            _ => {}
        }
...
async fn lock_cache_key(path: &str) -> tokio::sync::OwnedMutexGuard<()> {
    let lock = {
