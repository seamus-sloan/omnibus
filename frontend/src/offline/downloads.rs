...
fn last_stale_check() -> &'static std::sync::atomic::AtomicI64 {
...
pub(super) async fn refresh_stale_flags(server_url: &str) {
...
fn completed_validator_queries() -> Vec<omnibus_shared::DownloadValidatorQuery> {
    registry()
        .read()
        .map(|m| {
