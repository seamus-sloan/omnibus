//! Pure single-range `Range` header parser, shared by the server's byte-serving
//! endpoints and the mobile loopback media proxy so a range means the same
//! thing on both sides of a downloaded book. `<audio>`, epub.js, and the
//! download engine only ever send a single `bytes=` range, so multi-range
