...
#[test]
fn validate_rejects_more_measures_than_there_are_axes() {
    let s = spec(
        vec![
    ...
    assert_eq!(s.validate(), Err(ChartSpecError::TooManyMeasures));
}
...
fn validate_rejects_the_same_measure_plotted_twice() {
    ...
    assert_eq!(s.validate(), Err(ChartSpecError::DuplicateMeasures));
}
