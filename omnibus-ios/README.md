...
> **Releasing signs automatically.** TestFlight hands `xcodebuild` the App Store
> Connect key and `-allowProvisioningUpdates`, so Xcode reconciles the App IDs
> and profiles against the `.entitlements` files here — there are no profile
> secrets to keep current. That matters because a profile covers one bundle id,
> so an app with an embedded extension otherwise needs one hand-maintained
> profile per bundle, regenerated by hand on every capability change. Getting
> that wrong was silent: entitlements came from the profile, so one missing the
> App Group shipped a widget that installed and rendered nothing. CI re-reads the
> exported IPA to confirm the group landed. `just ios-build` / `just ios-test`
> cannot catch it — simulator builds sign ad-hoc.
