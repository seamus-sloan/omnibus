...
struct AuthorsView: View {
    ...
    private func load(force: Bool = false) async {
        ...
        do {
            authors = try await LibraryService.authors()
            error = nil
        } catch {
...
struct AuthorDetailView: View {
    ...
    private func load() async {
        do {
            detail = try await LibraryService.author(id: id)
            error = nil
        } catch {
            self.error = (error as? APIError)?.errorDescription ?? error.localizedDescription
        }
...
struct SeriesIndexView: View {
    ...
    var body: some View {
        ...
        .task {
            series = (try? await LibraryService.series()) ?? []
            isLoading = false
        }
    }
...
struct SeriesDetailView: View {
    ...
    var body: some View {
        ...
        .navigationBarTitleDisplayMode(.inline)
        .task {
            detail = try? await LibraryService.series(id: id)
            isLoading = false
        }
    }
...
struct TagCloudView: View {
    ...
    var body: some View {
        ...
        .navigationBarTitleDisplayMode(.inline)
        .task {
            tags = ((try? await LibraryService.tags()) ?? []).sorted { $0.count > $1.count }
            isLoading = false
        }
    }
}
