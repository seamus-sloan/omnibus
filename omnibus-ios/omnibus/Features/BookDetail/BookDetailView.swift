...
final class BookDetailModel {
    ...
    func load(uuid: String) async {
        ...
        async let ratingTask = UserDataService.rating(uuid: uuid)
        async let othersTask = UserDataService.otherRatings(uuid: uuid)
        async let statusTask = UserDataService.readStatus(uuid: uuid)
        async let journalsTask = UserDataService.journals(uuid: uuid)
        async let highlightsTask = UserDataService.highlights(uuid: uuid)
        async let bookmarksTask = UserDataService.bookmarks(uuid: uuid)
        async let shelvesTask = UserDataService.shelvesContaining(uuid: uuid)

        rating = await ratingTask?.stars ?? 0
        otherRatings = await othersTask
        readStatus = await statusTask?.status ?? .unread
        journals = await journalsTask
        highlights = await highlightsTask
        bookmarks = await bookmarksTask
        shelvesContaining = await shelvesTask
    }
...
struct BookDetailView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showShelfPicker, onDismiss: {
            Task { model.shelvesContaining = await UserDataService.shelvesContaining(uuid: uuid) }
        }) {
            if let book = model.book {
    ...
    private func downloadControl(_ book: Book) -> some View {
        let record = downloads.record(for: book.uuid)
        ...
        case .complete:
            HStack(spacing: Spacing.sm) {
                ...
                Button("Remove") {
                    Task { await downloads.remove(book.uuid) }
                }
                .font(.ui(13))
        ...
        case .running, .queued:
            HStack(spacing: Spacing.sm) {
                ProgressView(value: record?.fraction ?? 0)
                    ...
                    .frame(width: 90)
                Button("Cancel") { Task { await downloads.cancel(book.uuid) } }
                    .font(.ui(13))
                    .foregroundStyle(palette.ink3Color)
            }
        ...
        default:
            Button {
                Haptics.tap()
                Task { await downloads.start(book: book) }
            } label: {
                HStack(spacing: 5) {
                ...
                VStack(alignment: .leading, spacing: Spacing.xl) {
                    ForEach(model.journals) { entry in
                        JournalCard(entry: entry) {
                            ...
                            Task {
                                await UserDataService.deleteJournal(
                                    id: entry.id, bookUUID: book.uuid
                                )
    ...
    private func shareBook(_ book: Book) async {
        ...
        var url = downloads.localURL(for: book.uuid)
        if url == nil {
