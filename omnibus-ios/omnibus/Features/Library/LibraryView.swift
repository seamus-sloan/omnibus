...
final class LibraryModel {
    ...
    /// `formats` filter; `downloaded` has no server equivalent and is applied
    /// over the loaded page.
    var category: LibraryCategory = .all {
        didSet {
            guard category != oldValue else { return }
            if category.serverFormats != oldValue.serverFormats {
                Task { await reload() }
            }
        }
    }

    /// What the grid actually renders, after any client-side category filter.
    var visibleBooks: [Book] {
        guard category == .downloaded else { return books }
        let downloads = DownloadManager.shared
        return books.filter { downloads.isDownloaded($0.uuid) }
    }
    ...
    func reload() async {
        ...
        async let pageTask = LibraryService.page(
            sort: sort, direction: direction, formats: category.serverFormats, cursor: nil
        )
        async let resumeTask = try? await UserDataService.recentProgress()
        async let shelvesTask = try? await UserDataService.shelfPreviews()
        ...
        // Only the user's own shelves belong on the landing rail; a public
        // shelf from another account is browsable but isn't "yours".
        shelves = ((await shelvesTask) ?? []).filter { $0.shelf.bookCount > 0 || $0.shelf.kind != .wishlist }
    ...
    func refreshResume() async {
        resume = (try? await UserDataService.recentProgress()) ?? []
    }
    ...
    func loadMoreIfNeeded(currentItem: Book) async {
        ...
        do {
            let page = try await LibraryService.page(
                sort: sort, direction: direction, formats: category.serverFormats, cursor: cursor
            )
...
struct BookGridCell: View {
    ...
    private var badges: some View {
        HStack(spacing: 4) {
            ...
            if downloads.isDownloaded(book.uuid) {
                badge("arrow.down")
            }
        }
        .padding(6)
