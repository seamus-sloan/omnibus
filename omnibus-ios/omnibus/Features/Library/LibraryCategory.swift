...
enum LibraryCategory: Hashable, CaseIterable {
    ...
    /// filter — `downloaded` has no server equivalent and is answered from the
    /// local library mirror joined against the download registry.
    ...
    /// What this bucket means to a library read.
    var filter: LibraryFilter {
        LibraryFilter(formats: serverFormats, downloadedOnly: self == .downloaded)
    }
