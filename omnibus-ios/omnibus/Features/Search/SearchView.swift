...
struct SearchView: View {
    ...
    /// palette endpoint is rate-limited server-side.
    private func schedule(_ value: String) {
        ...
        searchTask = Task {
            ...
            let fetched = try? await LibraryService.searchPalette(query: trimmed)
            guard !Task.isCancelled else { return }
            results = fetched ?? PaletteResults()
        }
    }
}
...
struct SearchResultsView: View {
    ...
    var body: some View {
        ...
        .task {
            books = ((try? await LibraryService.searchFull(query: query))?.books) ?? []
            isLoading = false
        }
    }
}
