...
/// One reader's `(status, half_stars)` on a book, each `None` where the row is
/// absent — the shape every assertion below compares.
async fn curation_of(
    pool: &sqlx::SqlitePool,
    user: i64,
    book_uuid: &str,
) -> (Option<String>, Option<i64>) {
    let status: Option<String> = sqlx::query_scalar(
        "SELECT status FROM book_read_status WHERE user_id = ? AND book_uuid = ?",
    )
    .bind(user)
    .bind(book_uuid)
    .fetch_optional(pool)
    ...
    let rating: Option<i64> = sqlx::query_scalar(
        "SELECT half_stars FROM user_ratings WHERE user_id = ? AND book_uuid = ?",
    )
    .bind(user)
    .bind(book_uuid)
    .fetch_optional(pool)
...
fn curated(status: &str, half_stars: i64) -> (Option<String>, Option<i64>) {
    (Some(status.to_string()), Some(half_stars))
}
...
async fn undo_merge_returns_curation_to_each_book_when_the_absorbed_side_won() {
    ...
    assert_eq!(
        curation_of(&pool, user, &target).await,
        curated("reading", 8),
        "the survivor must get its own pre-merge curation back"
    );
    assert_eq!(
...
async fn undo_merge_clears_the_survivors_curation_when_only_the_absorbed_book_had_any() {
    ...
    assert_eq!(curation_of(&pool, user, &target).await, (None, None));
    assert_eq!(
        curation_of(&pool, user, &source).await,
        curated("finished", 5)
    );
...
async fn undo_merge_leaves_a_reader_the_merge_never_moved_a_row_for_alone() {
    ...
    let pool = init_db("sqlite::memory:").await.unwrap();
    let mover = seed_user(&pool).await;
    let bystander: i64 = sqlx::query_scalar(
        "INSERT INTO users (username, password_hash, is_admin) VALUES ('reader', 'x', 0) RETURNING id",
    )
    .fetch_one(&pool)
    .await
    .unwrap();
    ...
    assert_eq!(curation_of(&pool, mover, &target).await, (None, None));
}
...
async fn seed_identifier(pool: &sqlx::SqlitePool, book_uuid: &str, scheme: &str, value: &str) {
    let id = book_id_by_uuid(pool, book_uuid).await;
    sqlx::query("INSERT INTO book_identifiers (book_id, scheme, value) VALUES (?, ?, ?)")
        .bind(id)
        .bind(scheme)
        .bind(value)
        .execute(pool)
...
async fn undo_merge_takes_back_only_the_identifiers_the_merge_added() {
    ...
    assert_eq!(
        ...
        vec![
            ("amazon".to_string(), "shared".to_string()),
            ("isbn".to_string(), "111".to_string()),
            ("isbn".to_string(), "999".to_string()),
        ]
    );
...
#[tokio::test]
async fn undo_merge_refuses_when_the_survivor_was_recurated_after_the_merge() {
    ...
    // basis to choose, so it fails loudly and changes nothing rather than
    ...
    let out = merge_books(&pool, &source, &target, Some(user))
        ...
        .unwrap();
    sqlx::query(
        "UPDATE book_read_status SET status = 'finished', updated_at = 9000 WHERE book_uuid = ?",
    )
    .bind(&target)
    .execute(&pool)
    .await
...
#[tokio::test]
async fn undo_merge_leaves_curation_alone_for_a_merge_log_predating_the_snapshot() {
    ...
    obj.remove("curation");
    obj.remove("identifiers_added_to_target");
    sqlx::query("UPDATE merge_log SET source_metadata = ? WHERE id = ?")
