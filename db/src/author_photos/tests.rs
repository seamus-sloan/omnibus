...
// ── Host allowlist, HTTPS-only, and bounded redirects (provider covers) ──
//
...
#[tokio::test]
async fn fetch_remote_image_refuses_a_redirect_that_downgrades_to_http() {
    let server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/start.jpg"))
        .respond_with(
            ResponseTemplate::new(302).insert_header("Location", "http://127.0.0.1:9/final.jpg"),
        )
        .mount(&server)
        .await;

    // The first hop is reached over http too, so `require_https` can't be on
    // for the original; instead assert the *scheme gate runs per hop* by
    // pointing an https-required config at an https-shaped first URL.
    let cfg = RemoteImageConfig {
        allow_private_addresses: true,
        host_allowlist: vec!["127.0.0.1".to_string()],
        require_https: true,
