...
//! its own group, then any remaining `.mp3` files in the same directory
//! form a sibling group. The cases we've observed in practice never mix
//! the two, but the rule keeps the grouping deterministic if they do.
...
pub fn group_into_books(
    ...
    // first as one-entry groups, then bucket the remaining `.mp3` rows by
    // parent directory.
