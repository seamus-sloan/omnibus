...
/// `epub:type` values that mark a document as navigation rather than reading.
...
/// available.
const NAV_EPUB_TYPES: [&str; 3] = ["toc", "landmarks", "page-list"];
...
fn is_navigation_document(html: &str) -> bool {
    NAV_EPUB_TYPES.iter().any(|kind| {
        html.contains(&format!("epub:type=\"{kind}\""))
            || html.contains(&format!("epub:type='{kind}'"))
    })
}
